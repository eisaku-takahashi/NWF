...
Updated: 2026-04-04T16:00:00+09:00
...
また本ドキュメントは NWF Kernel Audit System と連携し、すべての Engine 実行が監査ログへ記録されることを前提とした Engine 実行ライフサイクル仕様の基準ドキュメントでもある。
...
## 4. Engine Execution Lifecycle & Audit Logging

NWF v2.0.1 ではすべての Engine は Kernel Audit System と連携し、以下のライフサイクルイベントを必ず発行しなければならない。

### 4.1 Engine Lifecycle Events

各 Engine は以下のイベントを Audit Logger に送信する。

- ENGINE_START
- ENGINE_FINISH
- ENGINE_ERROR
- ENGINE_STATE_UPDATE
- ENGINE_OUTPUT_GENERATED

### 4.2 Standard Audit Hook

各 Engine 実装は以下の監査フックを持つこと。
...
Story Engine は全 Engine の監査イベントを統合し、transaction 単位で Audit Trail を構築する。

---

## 5. Operational Pipeline（Execution Workflow）
...
すべてのステップは Audit Log に記録され、Pipeline Execution Trace として保存される。

---

## 6. Inter-Engine Dependencies
...
### 6.1 因果伝搬ルール（Causality Propagation）
...
State 変更イベントはすべて Audit Log に記録され、因果関係の追跡が可能でなければならない。

---

## 7. Engine Registry & Versioning
...
また Engine Version と Audit Log は commit_hash および spec_version と紐付けて記録される。
...
- Kernel Audit System がすべての Engine 実行を監査記録する
...
「監査可能な実行単位（Auditable Execution Unit）」
